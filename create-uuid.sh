...
if [ -f /boot/flightsurf-config.txt ]; then
    UUID_FILE="/boot/flightsurf-uuid"
else
    mkdir -p /usr/local/share/flightsurf
    UUID_FILE="/usr/local/share/flightsurf/flightsurf-uuid"
    # move old file position
    if [ -f /boot/flightsurf-uuid ]; then
        mv -f /boot/flightsurf-uuid $UUID_FILE
    fi
fi
